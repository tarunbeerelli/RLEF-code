...
nvidia-smi --query-gpu=name,memory.total --format=csv,noheader[cite: 9]

echo "[2/5] Environment & Dependency Setup..."
# Install and configure Poetry strictly to the local project
pip install poetry[cite: 9]
poetry config virtualenvs.in-project true[cite: 9]

# Clean up standard cloud image conflicts if they exist (prevents silent CUDA clashes)
sudo pip uninstall xgboost transformer_engine flash_attn pynvml opencv-python-headless -y || true[cite: 9]
...
bash scripts/download_apps.sh[cite: 9]
...
poetry run wandb login[cite: 9]

echo "HuggingFace Hub Login:"
HF_HUB_ENABLE_HF_TRANSFER=1 poetry run huggingface-cli login[cite: 9]
#hf auth login
